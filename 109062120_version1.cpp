...
public:
	...
	int setHeuristic() {
		...
		if (board[0][0] == 3- player && board[1][1] ==3-  player) {
			...
			if (board[1][6] == 3-player)heuristic -= 60;
			if (board[1][7] == 3-player || board[0][6] == 3-player)heuristic -= 50;
		}
...
int search_minimax(OthelloBoard now, int depth, int Alpha, int Beta) {
	...
	if (now.cur_player == player) {//自己找最大
		...
		for (auto test : now.next_valid_spots) {
			...
			if (next.put_disc(test)) {
				int eval = search_minimax(next, depth - 1, Alpha, Beta);
				if (eval < minEval && depth == MAXDEPTH) {
					been.x = test.x;
					been.y = test.y;
				}
